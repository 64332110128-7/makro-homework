/*
  ...
  - Added the required column `band_id` to the `Product` table without a default value. This is not possible if the table is not empty.
  - Added the required column `category_id` to the `Product` table without a default value. This is not possible if the table is not empty.

*/
-- AlterTable
ALTER TABLE `product` ADD COLUMN `band_id` INTEGER NOT NULL,
...
-- AddForeignKey
ALTER TABLE `Product` ADD CONSTRAINT `Product_band_id_fkey` FOREIGN KEY (`band_id`) REFERENCES `Brand`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;
